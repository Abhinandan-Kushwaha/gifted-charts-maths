...
export const getPieChartMainProps = (props: PieChartMainProps) => {
  ...
  let tooltipSelectedIndex = -1
  const setTooltipSelectedIndex = (x:number) => {
    tooltipSelectedIndex = x
  }
  ...
  const timer = { current: setTimeout(() => {}) } // timer for tooltip (it was useRef here in React)
