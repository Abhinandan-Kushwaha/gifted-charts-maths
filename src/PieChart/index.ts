...
interface IusePieChart {
  ...
  strokeWidth: number
  innerRadius: number
  innerCircleColor: any
  innerCircleBorderWidth: number
  innerCircleBorderColor: any
...
export const usePieChart = (props: IPieChartPropsType): IusePieChart => {
  ...
  let selectedIndex = props.focusedPieIndex ?? -1 // at the start, nothing is selected
  ...
  const initialiseTotalAndAngles = () => {
  ...
  initialiseTotalAndAngles()

  const initialiseNewStartAngle = () => {
    if (selectedIndex !== -1) {
      const newStartAngle = props.initialAngle ?? (props.semiCircle ? -pi : 0)
      let start = 0
